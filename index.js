...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    if (!process.env.STRIPE_WEBHOOK_SECRET) {
      ...
      return res.status(500).send("Webhook Error: Server is not configured to verify Stripe signatures.");
    }
    if (!sig) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    console.log("Stripe checkout completed for user:", userId, "email:", userEmail);
...
const verifyToken = async (req, res, next) => {
  ...
  try {
    // NOTE: never log the raw token or decoded payload here — doing so
    // leaks valid bearer credentials into server/cloud logs on every
    // authenticated request.
    const { payload } = await jwtVerify(token, JWKS);
    
    // Fetch latest user details from DB to bypass stale JWT token values
    const db = client.db("wisperia");
    const userCollection = db.collection('user');
    const dbUser = await userCollection.findOne({ _id: new ObjectId(payload.id) });
...
async function run() {
  try {
    ...
    app.post('/create-checkout-session', writeLimiter, async (req, res) => {
      ...
      if (!userId || !userEmail) {
        return res.status(400).json({ error: "Missing required user metadata" });
      }
    ...
    app.post('/subscriptions', async (req, res) => {
      try {
        ...
        if (!sessionId || !userId) {
          return res.status(400).json({ error: "Missing required sessionId or userId" });
        }
        ...
        if (verifiedSession.status !== "complete" || verifiedSession.payment_status !== "paid") {
    ...
    app.post('/user/add-lesson', verifyToken, async (req, res) => {
      try {
        ...
        const accessLevelValue = req.user.isPremium ? (data.accesslevel || data.accessLevel || "free") : "free";
    ...
    app.get('/public-lessons', async (req, res) => {
      try {
        const { search, category, emotionalTone, sort, page = 1, limit = 6 } = req.query;
        ...
        if (emotionalTone && emotionalTone !== "All") {
          query.$or = query.$or || [];
          query.$or.push({ emotionalTone: emotionalTone }, { emotionaltone: emotionalTone });
        }
        ...
        const lessons = await lessonCollection.find(query).sort(sortQuery).skip(skipNum).limit(limitNum).toArray();
    ...
    app.put('/lessons/:id', verifyToken, async (req, res) => {
      try {
        ...
        const existingLesson = await lessonCollection.findOne({ _id: new ObjectId(id) });
        if (!existingLesson) {
          return res.status(404).json({ error: "Lesson not found" });
        }

        // Only owners or admins can edit
        if (existingLesson.userId !== req.user.id && req.user.role !== "admin") {
    ...
    app.post('/lessons/:id/report', writeLimiter, verifyToken, async (req, res) => {
      try {
        const { id } = req.params;
    ...
    app.get('/favorites', verifyToken, async (req, res) => {
      try {
        ...
        const lessons = await lessonCollection.find({ _id: { $in: lessonIds } }).toArray();
        res.json(lessons);
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    ...
    app.post('/lessons/:id/comments', writeLimiter, verifyToken, async (req, res) => {
      try {
        const { id } = req.params;
        const { text } = req.body;
    ...
    app.get('/lessons/:id/comments', async (req, res) => {
      try {
        const { id } = req.params;
        const comments = await commentsCollection.find({ lessonId: id }).sort({ createdAt: -1 }).toArray();
        res.json(comments);
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    ...
    app.get('/user/dashboard-stats', verifyToken, async (req, res) => {
      try {
        ...
        const recentLessons = await lessonCollection.find({ userId }).sort({ createdAt: -1 }).limit(5).toArray();

        // Categorized distribution for simple visualization chart
        const categoryStats = await lessonCollection.aggregate([
          { $match: { userId } },
    ...
    app.get('/user/public-lessons/:userId', async (req, res) => {
      try {
        ...
        const lessons = await lessonCollection.find({ userId, visibility: "public" }).sort({ createdAt: -1 }).toArray();
        res.json(lessons);
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    ...
    app.get('/user/my-lessons', verifyToken, async (req, res) => {
      try {
        const lessons = await lessonCollection.find({ userId: req.user.id }).sort({ createdAt: -1 }).toArray();
        res.json(lessons);
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    ...
    app.get('/admin/stats', verifyToken, adminVerify, async (req, res) => {
      try {
        ...
        const todayNewLessons = await lessonCollection.countDocuments({ createdAt: { $gte: startOfToday } });

        // Most active contributors (top 5 users by created lessons)
        const activeContributors = await lessonCollection.aggregate([
          { $group: { _id: "$userId", count: { $sum: 1 }, name: { $first: "$creatorName" }, email: { $first: "$creatorEmail" } } },
          { $sort: { count: -1 } },
          { $limit: 5 }
        ]).toArray();
    ...
    app.get('/admin/users', verifyToken, adminVerify, async (req, res) => {
      try {
        ...
        const usersWithCount = await Promise.all(users.map(async (u) => {
          const count = await lessonCollection.countDocuments({ userId: u._id.toString() });
          return {
            ...u,
            lessonsCount: count
          };
    ...
    app.put('/admin/users/:id/role', verifyToken, adminVerify, async (req, res) => {
      try {
        const { id } = req.params;
        const { role } = req.body;
    ...
    app.put('/admin/lessons/:id/featured', verifyToken, adminVerify, async (req, res) => {
      try {
        const { id } = req.params;
    ...
    app.put('/admin/lessons/:id/reviewed', verifyToken, adminVerify, async (req, res) => {
      try {
        const { id } = req.params;
        const result = await lessonCollection.updateOne(
          { _id: new ObjectId(id) },
    ...
    app.get('/admin/reported-lessons', verifyToken, adminVerify, async (req, res) => {
      try {
        // Group reports by lessonId
        const reportGroups = await reportsCollection.aggregate([
          {
            $group: {
              _id: "$lessonId",
              count: { $sum: 1 },
              lessonTitle: { $first: "$lessonTitle" },
              reports: { $push: { reporterUserId: "$reporterUserId", reportedUserEmail: "$reportedUserEmail", reason: "$reason", timestamp: "$timestamp" } }
            }
          }
        ]).toArray();
    ...
    app.post('/admin/reported-lessons/:id/ignore', verifyToken, adminVerify, async (req, res) => {
      try {
        const { id } = req.params;
    ...
    app.get('/featured-lessons', async (req, res) => {
      try {
        const lessons = await lessonCollection.find({ isFeatured: true, visibility: "public" }).toArray();
        res.json(lessons);
      } catch (err) {
        res.status(500).json({ error: err.message });
      }
    ...
    app.get('/home-stats', async (req, res) => {
      try {
        ...
        const mostSaved = await lessonCollection.find({ visibility: "public" }).sort({ likesCount: -1 }).limit(6).toArray();
